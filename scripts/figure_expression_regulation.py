...
ax2.set_title(r'acetate $\rightarrow$ glucose', size=fontsize)
ax2.set_xlim(-1.1, 2.1)
ax2.set_xticks(np.arange(-1,2.1, 0.5))
...
ax3.set_title(r'chemostat: slow $\rightarrow$ fast', size=fontsize)
ax3.set_ylim(0, 30)
ax3.set_xlabel(r'$\Delta \log_2 (E) / \Delta \log_2 (v)$', size=fontsize)
...
sp=plt.scatter(dv.median(), dp.median(), c='#438343', 
                 s=30, marker='o',
                 vmin=0, vmax=1) 
...
#
plt.errorbar(dv.median(), dp.median(), yerr=yerr, xerr=xerr, zorder=0, lw=1.2, c='0.5', fmt='o', ms=0.1) 
#
axrng = [-0.3,2.5]
